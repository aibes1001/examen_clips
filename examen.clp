...
(defrule recojer_cajas_naranjas
    ...
    (pedido $?lp1 cajas_naranjas ?p $?lp1)
    (max ?m)
    (and (test (> ?s ?p)) (test (< ?pa ?p)))

=>
    (assert (robot posicion ?x carga cajas_naranjas (max (- ?p ?pa) ?m) pedido_acumulado $?l1 cajas_naranjas ?pa $?l2 palet_naranjas posicion ?x stock (- ?s (max (- ?p ?pa) ?m)) $?l3)
))
...
=>
    (assert (robot posicion ?x carga cajas_caquis (max (- ?p ?pa) ?m) pedido_acumulado $?l1 cajas_caquis ?pa $?l2 palet_caquis posicion ?x stock (- ?s (max (- ?p ?pa) ?m)) $?l3)
))
...
=>
    (assert (robot posicion ?x carga cajas_uvas (max (- ?p ?pa) ?m) pedido_acumulado $?l1 cajas_uvas ?pa $?l2 palet_uvas posicion ?x stock (- ?s (max (- ?p ?pa) ?m)) $?l3)
))
...
=>
    (assert (robot posicion ?x carga cajas_manzanas (max (- ?p ?pa) ?m) pedido_acumulado $?l1 cajas_manzanas ?pa $?l2 palet_manzanas posicion ?x stock (- ?s (max (- ?p ?pa) ?m)) $?l3)
))
